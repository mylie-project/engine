...
class TimerTest {
    ...
    void testOnNewFrameUpdatesFrameTime() throws InterruptedException {
        ...
        // Check that simulation delta equals real-time delta with the default modifier of 1.0.
        assertEquals(
            Time.frameTime().delta(),
            Time.frameTime().deltaSim(),
            1e-6,
    ...
    private float getSimTimeModifier(Timer timer) {
        ...
        // For example, if there's a getSimTimeModifier() method, you could call it here:
        // return timer.getSimTimeModifier();
        // If it's only internally set with Lombok @Setter, create a suitable getter or test via reflection.
