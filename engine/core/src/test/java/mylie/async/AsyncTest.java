...
public class AsyncTest {
    static Stream<Scheduler> schedulerProvider() {
        return Stream.of(Schedulers.singleThreaded(), Schedulers.virtualThreads(), Schedulers.forkJoin(), Schedulers.threadPool(1), Schedulers.workStealing(1));
    }
    ...
    void testDifferentCaches(Scheduler scheduler) {
        ...
        resultsNoCache.add(Async.async(Async.ExecutionMode.ASYNC, Async.Target.Any, Caches.No, 0, atomicIntegerIncrease, atomicInteger));
        resultsNoCache.add(Async.async(Async.ExecutionMode.ASYNC, Async.Target.Any, Caches.No, 0, atomicIntegerIncrease, atomicInteger));
        ...
        resultsOneFrame.add(Async.async(Async.ExecutionMode.ASYNC, Async.Target.Any, Caches.OneFrame, 0, atomicIntegerIncrease, atomicInteger));
        resultsOneFrame.add(Async.async(Async.ExecutionMode.ASYNC, Async.Target.Any, Caches.OneFrame, 0, atomicIntegerIncrease, atomicInteger));
        Wait.wait(resultsOneFrame);
        assertEquals(1, atomicInteger.get(), "Caches.OneFrame should cache results, only one unique task should execute.");
    ...
    void testOneFrameCache(Scheduler scheduler) {
        ...
        resultsOneFrame = new ArrayList<>();
        resultsOneFrame.add(Async.async(Async.ExecutionMode.ASYNC, Async.Target.Any, Caches.OneFrame, 0, atomicIntegerIncrease, atomicInteger));
        resultsOneFrame.add(Async.async(Async.ExecutionMode.ASYNC, Async.Target.Any, Caches.OneFrame, 1, atomicIntegerIncrease, atomicInteger));
        Wait.wait(resultsOneFrame);
    ...
    void simpleAsync(Scheduler scheduler) {
        Async.initialize(scheduler);
        AtomicInteger atomicInteger = new AtomicInteger(0);
        Wait.wait(Async.async(Async.ExecutionMode.ASYNC, Async.Target.Any, Caches.No, 0, atomicIntegerIncrease, atomicInteger));
        assertEquals(1, atomicInteger.get());
        Async.shutdown();
    }
    ...
    void testCache(Scheduler scheduler) {
        ...
        results.add(Async.async(Async.ExecutionMode.ASYNC, Async.Target.Any, Caches.OneFrame, 0, atomicIntegerIncrease, atomicInteger));
        results.add(Async.async(Async.ExecutionMode.ASYNC, Async.Target.Any, Caches.OneFrame, 0, atomicIntegerIncrease, atomicInteger));
        Wait.wait(results);
        assertEquals(1, atomicInteger.get());
        Async.shutdown();
    ...
    void testAtomicIntegerDecrease(Scheduler scheduler) {
        Async.initialize(scheduler);
        AtomicInteger atomicInteger = new AtomicInteger(0);
        Wait.wait(Async.async(Async.ExecutionMode.ASYNC, Async.Target.Any, Caches.No, 0, atomicIntegerDecrease, atomicInteger));
        assertEquals(-1, atomicInteger.get());
        Async.shutdown();
    }
    ...
    void testThrowException(Scheduler scheduler) {
        ...
        assertThrows(RuntimeException.class, () -> {
            Wait.wait(Async.async(Async.ExecutionMode.ASYNC, Async.Target.Any, Caches.No, 0, throwException, atomicInteger));
        });
        Async.shutdown();
    }
    ...
    void testSequentialAsync(Scheduler scheduler) {
        ...
        List<Result<Boolean>> results = new ArrayList<>();
        for (int i = 0; i < 5; i++) {
            results.add(Async.async(Async.ExecutionMode.ASYNC, Async.Target.Any, Caches.No, 0, atomicIntegerIncrease, atomicInteger));
        }
        Wait.wait(results);
    ...
    private static final Function.F1<AtomicInteger, Boolean> atomicIntegerIncrease = new Function.F1<>("AtomicIntegerIncrease") {
        @Override
