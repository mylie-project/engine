...
public interface Timer extends Components.CoreComponent {
	/// Retrieves the current time state managed by the Timer. The time state
	/// contains
	/// information such as version, delta, and modified delta values, which
	/// represent
	/// the elapsed time and its transformations.
	///
	/// @return the current instance of the Time object representing the Timer's
	/// state.
	Time time();
	...
	/// Represents a state of time with associated versioning and transformation
	/// data.
